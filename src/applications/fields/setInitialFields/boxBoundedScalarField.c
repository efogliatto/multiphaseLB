#include <io.h>
#include <latticeMesh.h>
#include <stdlib.h>
#include <stdio.h>


void boxBoundedScalarField( struct latticeMesh* mesh, double** field, char* fname ) {


    // Allocate memory
    
    *field = (double*)malloc( mesh->mesh.nPoints * sizeof(double) );


    
    // Look for values

    char aux[100];

    double in = 0,
	out = 0;

    double min[3], max[3];


    // Min corner
    
    sprintf(aux, "%s/internalField/minPoint", fname);

    unsigned int status = lookUpVector("start/initialFields", aux, min, 3);


    // Max corner
    
    sprintf(aux, "%s/internalField/maxPoint", fname);

    status = lookUpVector("start/initialFields", aux, max, 3);
    


    // Inside value

    char ins[100];
    
    sprintf(ins, "%s/internalField/inside", fname);
    
    status = lookUpDouble("start/initialFields", ins, &in, 0);



    // Outside value
    
    sprintf(ins, "%s/internalField/outside", fname);
    
    status = lookUpDouble("start/initialFields", ins, &out, 0);

    
    
    if (status) {}
    
    
    uint ii;

    for( ii = 0 ; ii < mesh->mesh.nPoints ; ii++ ) {


	if( ( min[0] <= mesh->mesh.points[ii][0] )   &&
	    ( max[0] >= mesh->mesh.points[ii][0] )   &&
	    ( min[1] <= mesh->mesh.points[ii][1] )   &&
	    ( max[1] >= mesh->mesh.points[ii][1] )   &&
	    ( min[2] <= mesh->mesh.points[ii][2] )   &&
	    ( max[2] >= mesh->mesh.points[ii][2] )  ) {
	
	    field[0][ii] = in;

	}

	else {

	    field[0][ii] = out;

	}

    }
    


}
