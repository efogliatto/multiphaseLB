...
void fixedGradT( latticeMesh* mesh, macroFields* mfields, scalar** field, unsigned int bid, lbeModel model, bdParam* bp, lbParameters* lp ) {
	...
	/* srand( time(NULL) ); */
	srand( mesh->parallel.pid );

	scalar minT = bp->fgt.T * (100.0-bp->fgt.pert) / 100.0;

	scalar maxT = bp->fgt.T * (100.0+bp->fgt.pert) / 100.0;
